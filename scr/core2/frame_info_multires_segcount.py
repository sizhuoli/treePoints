...
class FrameInfo:
    ...
    def __init__(self, img1, img2, annotations, weight, density, dtype=np.float32):
    ...
    def getPatch(self, i, j, patch_size, img_size, normalize=1.0, gbnorm = 0):
        ...
        # do global normalize
        if gbnorm == 1:
            im1 = (im1 - np.array([[118.66, 121.69, 107.15, 178.43, 0]]))/ np.array([[29.32, 26.32, 25.42, 24.28, 1]])
            
            im2 = (im2 - np.array([[3.5]])) / np.array([[3]])
    ...
    def random_patch(self, patch_size, normalize, gbnorm):
        ...
        img_patch1, img_patch2 = self.getPatch(x, y, patch_size, ic, normalize, gbnorm)
